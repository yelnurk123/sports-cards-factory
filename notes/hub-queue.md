...
| M1.2 world pass (rebuild environment per env v1.1; per-base belts + vendors; box stacks; kill gap + tutorial overlap; **publish into the real place**) | tasked 2026-08-11-001 | build chat | game is currently only in a /tmp local-file build — the published "Sports Card Farm" experience may not even exist yet (user publishing manually; task addendum covers it). **Flow v1.3 addendum in the task file: per-plot Sell vendor + whole-stack carry** |
| M2 task (leveling + full 45-rung ladder) | owed | M1.2 + user playtest + foil spec | gate order: M1.2 ships → user playtests the real place → M2 |
