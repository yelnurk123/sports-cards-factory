...
- **Revised scope:** finish BASEBALL (12 pending) then ESPORTS (40 pending). RACING moves to Lane A — do not touch manifest-racing.csv anymore.
- **Elevated style lock (spec v2.0, §7g):** cinematic grading (rim-light, moody contrast — the Lane B MMA look), one storytelling element per card, VFX REQUIRED at Icon+, hero prop per chaser, framing escalates with band, GOAT/Eternal = moment cards. Read the spec header — version changed, full re-read required.
...
**Launch prompt for a new Lane C chat:** "You are Lane C of Sports Legends Card Farm. Clone github.com/yelnurk123/sports-cards-factory (token provided separately, never print it). Read briefs/factory-lane-C-brief.md, then specs/sports-legends-art-spec-v1.md (v2.1), then your manifests. Continue from the first pending row in your scope. Follow those files exactly."
**Scope:** baseball (manifest-baseball.csv, 12 pending), then esports (manifest-esports.csv, 40 pending). Racing belongs to Lane A now; never touch manifest-racing.csv.
