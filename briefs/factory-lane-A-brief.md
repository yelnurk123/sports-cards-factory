...
**Launch prompt for a new Lane A chat:** "You are Lane A of Sports Legends Card Farm. Clone github.com/yelnurk123/sports-cards-factory (token provided separately, never print it). Read briefs/factory-lane-A-brief.md, then specs/sports-legends-art-spec-v1.md (v2.1), then your manifests. Continue from the first pending row in your scope. Follow those files exactly."
**Scope:** regen queue (glf-ms-1, glf-ms-3, soc-wc-1/2/4), then WWE (manifest-wwe.csv), then racing (manifest-racing.csv). Do NOT continue hockey or any new sport — those await a hub canon decision.
